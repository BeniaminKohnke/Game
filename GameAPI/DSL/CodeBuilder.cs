...
namespace GameAPI.DSL
{
    public static class CodeBuilder
    {
        ...
        private static string TranslateToCSharp(string scriptName, string code)
        {
            ...
            builder.AppendLine("public override void Do(GameWorld gameWorld, Dictionary<string, object> parameters){");
            builder.Append(code);
            builder.AppendLine("}}}");
        ...
        private static string TranslateVariables(string code)
        {
            foreach (var match in _variableRegex.Matches(code).Cast<Match>())
            {
                ...
                {
                    code = code.Replace(match.Value, '\"' + match.Groups[1].Value + '\"');
                }
            }
                ...
                while (index != -1)
                {
                    ...
                    if (_scriptFunctions.TryGetValue(functionName.Trim(), out var paramCount))
                    {
                        ...
                        if (parts.Length > lastIndex)
                        {
                            ...
                            var function = @$"{functionName}({(string.IsNullOrEmpty(parameters) ? string.Empty : $"{parameters},")}gameWorld,parameters,deltaTime)";
        ...
        private static string ChangeTabsToBrackets(string code)
        {
            ...
            while (codeGrid.Any(l => l.Any(p => p.Contains("\t"))))
            {
                ...
                if (firstGridIndex != secondGridIndex)
                {
                    ...
                    codeGrid[secondGridIndex][codeGrid[secondGridIndex].Length - 1] = codeGrid[secondGridIndex][codeGrid[secondGridIndex].Length - 1] + "}";
                }
                else
                {
                    codeGrid[firstGridIndex][indexOfTab] = "{";
                    codeGrid[firstGridIndex][codeGrid[firstGridIndex].Length - 1] = codeGrid[firstGridIndex][codeGrid[firstGridIndex].Length - 1] + "}";
                }
            }
